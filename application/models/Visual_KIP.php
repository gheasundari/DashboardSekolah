...
class Visual_KIP extends CI_Model
{
    ...
    {
        $data = $this->db->query("SELECT count(`fs`.`id_penerimakip`) as jumlah_kip, `kip`.`penerimakip`, `dt`.`tahun` FROM `fact_sekolah` `fs` LEFT JOIN `dim_tahun` `dt` ON `dt`.`id_tahun` = `fs`.`data_tahun` LEFT JOIN `dim_penerimakip` `kip` ON `kip`.`id_kip` = `fs`.`id_penerimakip` GROUP BY `dt`.`tahun`, `kip`.`penerimakip` HAVING `dt`.`tahun` = '" . $tahun . "'");
        return $data->result();
    ...
    {
        $data = $this->db->query("SELECT count(`fs`.`id_penerimakip`) as jumlah_kip, `dt`.`tahun` FROM `fact_sekolah` `fs` LEFT JOIN `dim_tahun` `dt` ON `dt`.`id_tahun` = `fs`.`data_tahun` LEFT JOIN `dim_penerimakip` `kip` ON `kip`.`id_kip` = `fs`.`id_penerimakip` GROUP BY `dt`.`tahun`, `kip`.`penerimakip` HAVING `dt`.`tahun` = '" . $tahun . "' and penerimakip='Ya'");
        return $data->row();
        // return $this->db->get_compiled_select();
    }
