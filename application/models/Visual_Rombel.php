...
class Visual_Rombel extends CI_Model
{
    ...
    {
        $data = $this->db->query("SELECT count(`fs`.`id_rombel`) as jumlah, `rb`.`nama_rombel` as rombel, `dt`.`tahun` FROM `fact_sekolah` `fs` LEFT JOIN `dim_tahun` `dt` ON `dt`.`id_tahun` = `fs`.`data_tahun` LEFT JOIN `dim_rombel` `rb` ON `rb`.`id_rombel` = `fs`.`id_rombel` GROUP BY `dt`.`tahun`, rombel  HAVING tahun <= " . ($tahun) . " order by tahun desc limit 6");
        return $data->result();
        // return $this->db->get_compiled_select();
    }
}
