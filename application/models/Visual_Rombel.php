...
class Visual_Rombel extends CI_Model
{
    ...
    {
        $data = $this->db->query("SELECT count(`fs`.`rombel`) as jumlah, `rb`.`nama_rombel`, `dt`.`tahun` FROM `fact_sekolah` `fs` LEFT JOIN `dim_tahun` `dt` ON `dt`.`id_tahun` = `fs`.`data_tahun` LEFT JOIN `dim_rombel` `rb` ON `rb`.`id_rombel` = `fs`.`rombel` GROUP BY `dt`.`tahun`, rombel HAVING `dt`.`tahun` = '" . ($tahun) . "'");
        return $data->result();
